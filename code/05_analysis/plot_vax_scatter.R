...
zscore <- function(x){ if (sum(is.finite(x))<2 || sd(x,na.rm=TRUE)==0) return(rep(NA_real_,length(x)))
...
d <- read.csv("output/descriptors_vax.csv", stringsAsFactors=FALSE) %>%
  ...
  group_by(country) %>%
  mutate(across(c(vax_cov_65, steepness, peak_week, peak_height, onset_week, auc), zscore)) %>% ungroup()
...
outcome_labels <- c(auc="AUC (log, z)", peak_height="peak height (log, z)", peak_week="peak week (z)",
            onset_week="onset week (z)", steepness="steepness (z)")
long$outcome <- factor(long$outcome, levels=names(outcome_labels), labels=outcome_labels)
cor_by_outcome <- long %>% group_by(outcome) %>% summarise(r=cor(vax_cov_65,y), n=n(), .groups="drop") %>%
...
p <- ggplot(long, aes(vax_cov_65, y)) +
  ...
  geom_text(data=cor_by_outcome, aes(x=-Inf, y=Inf, label=lab), hjust=-0.1, vjust=1.4, size=3.4, inherit.aes=FALSE) +
...
cat("figure -> output/vax_scatter.png\n"); print(as.data.frame(cor_by_outcome[,c("outcome","r","n")]), row.names=FALSE)
