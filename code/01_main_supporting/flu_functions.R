...
# ---- |-contact-matrix transform: 17-band reciprocity-corrected matrices -> 4 model age groups ----
transform_contacts = function(data, params) {

  contacts_by_country = list()
  ...
  for (country_i in xlocations$location_name){
    ...
    # Collapse 17 bands -> 4 model age groups (0-4 | 5-14 [bands 2:3] | 15-64 [4:13] | 65+ [14:17]).
    # contacts_total[i,j] counts contact-ends DIRECTED from band i to band j (each i-j event
    # appears once in [i,j] and once in [j,i]) -- the convention the reciprocity correction above
    # establishes. The merged per-person rate is (ends in block) / pop_group, so every merged
    # entry, diagonal blocks included, is the FULL block sum. (A lower-triangle sum here, as this
    # code once had with the reasoning 'don't count twice', drops half of the cross-band ends and
    # understates within-group mixing by up to ~1.8x for the 10-band 15-64 group.)
    ...
    #
    contacts_total_new[3,3] = sum(contacts_total[4:13,4:13])
    ...
    #
    contacts_total_new[4,4] = sum(contacts_total[14:17,14:17])
    ...
    #   * A population-weighted mean-contacts-per-person rescaling (once computed here, removed as
    #     unused) is only a proxy: it equals the spectral-radius normalisation only when the mean
    #     tracks the leading eigenvalue. For fixing R0 the eigenvalue normalisation is the exact
    #     one; do this when the age/vaccination-structured model is revisited.
    contacts_by_country[[country_i]] = contacts_recovered
  }

  # create EU average by summing all locations
  contacts_collect = contacts_by_country[[1]]*0
  collect_counter = 0
  for (country_i in names(contacts_by_country)) {
    contacts_collect = contacts_collect + contacts_by_country[[country_i]]
  ...
  contacts_by_country[["EU"]] = contacts_new
