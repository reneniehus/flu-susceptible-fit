...
fit_sir_deterministic = function(ylist, R0 = 1.5, infectious_period_days = 3, seed_i0 = 1e-5,
  ...
  base = c(rep(qlogis(0.8), K), rep(log(sg$ymax / peak_inc_guess), K), log(max(sg$bguess, 1e-3)), log(15))
  ...
  best = .fit_multistart(base, jit_sd, .det_negll,
                         list(ylist = ylist, R0 = R0, gamma = gamma, I0 = seed_i0, n_sub = n_sub),
                         n_starts, seed, "fit_sir_deterministic")
